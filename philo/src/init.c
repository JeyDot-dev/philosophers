...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/02 18:25:08 by jsousa-a          #+#    #+#             */
/*   Updated: 2023/12/02 19:23:29 by jsousa-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "philo.h"
t_locks	*malloc_mutexes(void)
{
	t_locks	*locks;
	...
	if (!locks)
		return (NULL);
	locks->l_forks = malloc(sizeof(pthread_mutex_t) * 2);
	if (!locks->l_forks)
	{
		free(locks);
		return (NULL);
	...
	locks->l_is_eating = malloc(sizeof(pthread_mutex_t));
	if (!locks->l_is_eating)
	{
		free(locks->l_print);
		free(locks->l_forks);
		free(locks);
